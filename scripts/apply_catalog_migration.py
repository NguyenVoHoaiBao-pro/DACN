"""Apply a SQL migration file to electro_catalog_db (reads repo .env)."""
import sys
...
if not sql_path or not sql_path.exists():
    print("Usage: python scripts/apply_catalog_migration.py database/migrations/<file>.sql")
    sys.exit(1)
...
conn.commit()
conn.close()
print("Migration complete:", sql_path.name)
